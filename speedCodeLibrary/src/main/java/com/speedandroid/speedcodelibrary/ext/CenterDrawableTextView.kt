...
class CenterDrawableTextView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (drawables[LEFT] != null) {
            ...
            drawables[LEFT]!!.setBounds(left, top, left + widths[LEFT], top + heights[LEFT])
            canvas.save()
        ...
        if (drawables[RIGHT] != null) {
            ...
            drawables[RIGHT]!!.setBounds(left, top, left + widths[RIGHT], top + heights[RIGHT])
            canvas.save()
        ...
        if (drawables[TOP] != null) {
            ...
            drawables[TOP]!!.setBounds(left, bottom - heights[TOP], left + widths[TOP], bottom)
            canvas.save()
        ...
        if (drawables[BOTTOM] != null) {
            ...
            drawables[BOTTOM]!!.setBounds(left, top, left + widths[BOTTOM], top + heights[BOTTOM])
            canvas.save()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        drawables[LEFT]?.let { requireWidth += widths[LEFT] + compoundDrawablePadding }
        drawables[RIGHT]?.let { requireWidth += widths[RIGHT] + compoundDrawablePadding }
        val newWidthMeasureSpec = resolveSizeAndState(requireWidth, widthMeasureSpec, 1)

        var requireHeight = measuredHeight
        drawables[TOP]?.let { requireHeight += heights[TOP] + compoundDrawablePadding }
        drawables[BOTTOM]?.let { requireHeight += heights[BOTTOM] + compoundDrawablePadding }
